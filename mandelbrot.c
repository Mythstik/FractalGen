...
void fractal(SDL_Renderer* renderer, int width, int height, float left, float top, float xside, float yside)
{
    float xscale = xside / (float) width;
    float yscale = yside / (float) height;
        ...
        while (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT) {
                ...
                switch (e.key.keysym.sym) {
                    case SDLK_UP:
                        move_view(&left, &top, &xside, &yside, 0, -20);
                        break;
                    case SDLK_DOWN:
                        move_view(&left, &top, &xside, &yside, 0, 20);
                        break;
                    case SDLK_LEFT:
                        move_view(&left, &top, &xside, &yside, -20, 0);
                        break;
                    case SDLK_RIGHT:
                        move_view(&left, &top, &xside, &yside, 20, 0);
                        break;
                    case SDLK_z:
                        zoom_in(&left, &top, &xside, &yside, 1.5f);
                        break;
                    case SDLK_x:
                        zoom_out(&left, &top, &xside, &yside, 1.5f);
                        break;
                    case SDLK_ESCAPE:
                        quit = true;
                        break;
